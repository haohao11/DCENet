...
def dynamic_maps(offsets, data, size, dist_thre=1.5, ratio=0.9, max_friends=100):
    '''
    ...
    # print('friends /n', friends[:, :5])
            
    for ego_data in offsets:
        o_map = get_dynamic_maps(ego_data, offsets, size, friends)
...
def get_dynamic_maps(ego_data, offsets, size, friends=None, islog=False, max_speed=6):
    '''
        ...
        for otherId in otherIds:
            if egoId != otherId:
                ...
                if delta_h<=height and delta_w<=width and delta_h>=0 and delta_w>=0:
                    ...
                    # Only consider the static position
                    # xl, xr = max(0, int(np.floor(delta_w-pad))), min(int(np.floor(delta_w+pad)), width)
                    # yl, yr = max(0, int(np.floor(delta_h-pad))), min(int(np.floor(delta_h+pad)), height)
