...
def main():
    ...
    parser.add_argument('--lr', type=float, default=3e-4, help='Learning rate') # train with 3e-4, fine-tune with 1e-4
    ...
    for path in train_paths:
        ...
        if not os.path.exists(pathlib.Path(__file__).parent / f"../processed_data/train/{dataname}.npz"):
            ...
            print('data not exist, start processing...')           
    ...
    if args.train_mode: 
        ...
        # # # ToDO, optimized this here
        # traindata_list = Datalist.train_data
        # # traindata_list = Datalist.train_merged
        # print("traindata_list", traindata_list)
        
        
        train_merged_exist = os.path.join(pathlib.Path(__file__).parent / "../processed_data/train", "train_merged.npz")            
        if not os.path.exists(train_merged_exist):
            traindata_list = Datalist.train_data               
        else:
            traindata_list = Datalist.train_merged
        print("traindata_list", traindata_list)
        ...
        train.load_weights(str((pathlib.Path(__file__).parent / "../models/trans_net_12_1000_20220922-222311.hdf5"))) 
        ...
        train.load_weights(pathlib.Path(__file__).parent / "../models/trans_net_12_1000_20220922-222311.hdf5")
    ...
    for i in range(len(Datalist.test_data)):
        ...
        writer.write_pred_txt(test_trajs[:, :args.obs_seq, :], predictions, 
                              str(args.pred_seq)+"_"+testdata_list[0], "results/DCENet")        
