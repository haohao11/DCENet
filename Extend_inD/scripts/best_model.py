...
#------------- Added transformer Encoder---------------------------------------------------------------------------------------
        self.sample_encoder1 = Encoder(32, 2, 32, 3)
        self.att_layer_in = self.sample_encoder1(self.occus_layers)
...
#----------------------------------------------------------------------------------------------------------------------------------
        self.interaction_model = Model(self.occus_in,
                                       self.occus_out)  # self.occus_out (None, 32)     self.att_layer.shape  (None, None, 4)
        ...
        ## self.x_state.shape (None, 64)    self.x_dense.shape)  (None, 7, 64)
        self.sample_encoder2 = Encoder(64, 8, 64, 3)

        self.x_dense_in = self.sample_encoder2(self.x_dense)
        ...
        # ------------------------------------------------------------------------------------------------------------------------------
        # ------------- Added transformer Encoder for the occupancy information ---------------------------------------------------------------------------------------
        self.sample_encoder3 = Encoder(32, 2, 32, 3)
        self.y_occus_out_in = self.sample_encoder3(self.y_occus_layers)
        ...
        # -------------------------------------------------------------------------------------------------------------------------------------------
        # self.y_state (None, 64)      self.y_dense (None, 12, 64)
        # #
        self.sample_encoder4 = Encoder(64, 8, 64, 3)
        self.y_dense_in = self.sample_encoder4(self.y_dense)
        ...
        self.z_decoder3 = LSTM(self.z_decoder_dim,
                               ...
                               name='z_decoder3')
        #-----------------------------------------------------------------------------------------------
        # self.z_decoder31 = Encoder(64, 8, 64, 3)
        ...
        self.z_d3 = self.z_decoder3(
            self.z_d2)  # self.z_d2.shape (None, 12, 256)            self.z_d3.shape (None, 12, 64)


        self.z_d4 = self.z_decoder4(self.z_d3)
        # self.z_d5 = self.z_decoder5(self.z_d4)
        self.y_prime = self.y_decoder(self.z_d4)
    ...
    def training(self):
        ...
        cvae = Model([self.occus_in, self.x, self.y_occus_in, self.y],
                     [self.y_prime])
        ...
        save_dir = '../models'
        filepath = os.path.join(save_dir, 'CVAE.pdf')
        #plot_model(cvae, to_file=filepath, show_shapes=True)
        return cvae
    ...
    def Decoder(self):
        ...
        print('Construct the Decoder for trajectory preidction')
